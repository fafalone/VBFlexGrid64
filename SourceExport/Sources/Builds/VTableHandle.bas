...
Public Function VTableCall(ByVal RetType As VbVarType, ByVal InterfacePointer As LongPtr, ByVal Entry As Long, ParamArray ArgList() As Variant) As Variant
...
If UBound(VarArgList) > -1 Then
    ...
    Next i
    HResult = DispCallFunc(InterfacePointer, (Entry - 1) * cbPtr, CC_STDCALL, RetType, i, VarPtr(ArrVarType(0)), VarPtr(ArrVarPtr(0)), VTableCall)
Else
    HResult = DispCallFunc(InterfacePointer, (Entry - 1) * cbPtr, CC_STDCALL, RetType, 0, 0, 0, VTableCall)
End If
...
If ObjectPointer <> 0 Then
    ...
    CopyMemory IUnk, ObjectPointer, cbPtr
    IUnk.Release
    CopyMemory IUnk, 0&, cbPtr
End If
...
If VTableIPAOData.RefCount > 0 Then
    ...
    Set .IOleIPAO = This
    End With
    CopyMemory ByVal VarPtr(PropOleInPlaceActiveObject), VarPtr(VTableIPAOData), cbPtr
...
If OriginalVTableControl = 0 Then CopyMemory OriginalVTableControl, ByVal ObjPtr(This), cbPtr
CopyMemory ByVal ObjPtr(This), ByVal VarPtr(GetVTableControl()), cbPtr
End Sub

Private Sub RestoreIOleControl(ByVal This As OLEGuids.IOleControl)
If OriginalVTableControl <> 0 Then CopyMemory ByVal ObjPtr(This), OriginalVTableControl, cbPtr
End Sub
...
If VTableControl(0) = 0 Then
    If OriginalVTableControl <> 0 Then
        CopyMemory VTableControl(0), ByVal OriginalVTableControl, (3 * cbPtr)
    Else
    ...
    If OriginalVTableControl <> 0 Then
        CopyMemory VTableControl(6), ByVal UnsignedAdd(OriginalVTableControl, (6 * cbPtr)), cbPtr
    Else
...
If OriginalVTableControl <> 0 Then
    ...
    CopyMemory IUnk, VarPtr(This), cbPtr
...
If OriginalVTableControl <> 0 Then
    Dim ShadowIOleControl As OLEGuids.IOleControl
    This = OriginalVTableControl
    CopyMemory ShadowIOleControl, VarPtr(This), cbPtr
    Original_IOleControl_GetControlInfo = ShadowIOleControl.GetControlInfo(CI)
    CopyMemory ShadowIOleControl, 0&, cbPtr
    This = GetVTableControl()
Else
...
If OriginalVTablePPB = 0 Then CopyMemory OriginalVTablePPB, ByVal ObjPtr(This), cbPtr
CopyMemory ByVal ObjPtr(This), ByVal VarPtr(GetVTablePPB()), cbPtr
End Sub

Private Sub RestoreIPPB(ByVal This As OLEGuids.IPerPropertyBrowsing)
If OriginalVTablePPB <> 0 Then CopyMemory ByVal ObjPtr(This), OriginalVTablePPB, cbPtr
End Sub
...
If VTablePPB(0) = 0 Then
    If OriginalVTablePPB <> 0 Then
        CopyMemory VTablePPB(0), ByVal OriginalVTablePPB, (3 * cbPtr)
    Else
    ...
    If OriginalVTablePPB <> 0 Then
        CopyMemory VTablePPB(4), ByVal UnsignedAdd(OriginalVTablePPB, (4 * cbPtr)), cbPtr
    Else
...
Private Function IPPB_GetDisplayString(ByRef This As LongPtr, ByVal DispID As Long, ByRef pDisplayName As LongPtr) As Long
...
Else
    ...
    pElems = CoTaskMemAlloc(cElems * cbPtr)
    ...
    For nElemCount = 0 To cElems - 1
        ...
        CopyMemory ByVal UnsignedAdd(pElems, nElemCount * cbPtr), ByVal VarPtr(lpString), cbPtr
    Next nElemCount
    pElems = CoTaskMemAlloc(cElems * cbPtr)
    pCaCookiesOut.cElems = cElems
    pCaCookiesOut.pElems = pElems
    For nElemCount = 0 To cElems - 1
        CopyMemory ByVal UnsignedAdd(pElems, nElemCount * cbPtr), CookiesOutArray(nElemCount), cbPtr
    Next nElemCount
...
Private Function Original_IPPB_GetDisplayString(ByRef This As LongPtr, ByVal DispID As Long, ByRef pDisplayName As LongPtr) As Long
If OriginalVTablePPB <> 0 Then
    Dim ShadowIPPB As OLEGuids.IPerPropertyBrowsing
    This = OriginalVTablePPB
    CopyMemory ShadowIPPB, VarPtr(This), cbPtr
    Original_IPPB_GetDisplayString = ShadowIPPB.GetDisplayString(DispID, pDisplayName)
    CopyMemory ShadowIPPB, 0&, cbPtr
    This = GetVTablePPB()
End If
End Function
...
If hMem <> 0 Then
    ...
    CopyMemory ByVal VarPtr(GetNewEnum), hMem, cbPtr
    CopyMemory ByVal VarPtr(.Enumerable), 0&, cbPtr
End If
End With
End Function
