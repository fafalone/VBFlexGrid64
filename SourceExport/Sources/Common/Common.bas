...
Private Declare PtrSafe Function SetClipboardData Lib "user32" (ByVal wFormat As Long, Optional ByVal hMem As LongPtr) As LongPtr
...
Private Declare PtrSafe Function GetObjectAPI Lib "gdi32" Alias "GetObjectW" (ByVal hObject As LongPtr, ByVal nCount As Long, lpObject As Any) As Long
...
Private Declare PtrSafe Function DrawIconEx Lib "user32" (ByVal hDC As LongPtr, ByVal xLeft As Long, ByVal yTop As Long, ByVal hIcon As LongPtr, ByVal cxWidth As Long, ByVal cyWidth As Long, ByVal istepIfAni As Long, ByVal hbrFlickerFreeDraw As LongPtr, ByVal diFlags As Long) As Long
...
CopyMemory TempUserControl, ObjPtr(UserControl), cbPtr
Set TopUserControl = TempUserControl
CopyMemory TempUserControl, NULL_PTR, cbPtr
With TopUserControl
If .ParentControls.Count > 0 Then
    ...
    If TypeOf .ParentControls(0) Is VB.VBControlExtender Then
        ...
        CopyMemory TempUserControl, ObjPtr(.ParentControls(0)), cbPtr
        Set TopUserControl = TempUserControl
        CopyMemory TempUserControl, NULL_PTR, cbPtr
...
Dim ptt As Currency
CopyMemory ptt, P, 8
If hWndCursor = 0 Then hWndCursor = WindowFromPoint(ptt)
...
End If
If hWndCursor <> 0 Then SendMessage hWndCursor, WM_SETCURSOR, hWndCursor, ByVal MakeDWord(CLng(SendMessage(hWndCursor, WM_NCHITTEST, 0, ByVal Make_XY_lParam(P.X, P.Y))), WM_MOUSEMOVE)
End Sub
...
CopyMemory TempObj, ObjectPointer, cbPtr
Set PtrToObj = TempObj
CopyMemory TempObj, 0&, cbPtr
End Function
# Else
...
#If VBA7 Then
Public Function LoWord(ByVal Param As LongPtr) As Integer
    CopyMemory LoWord, ByVal VarPtr(Param), 2
End Function
Public Function HiWord(ByVal Param As LongPtr) As Integer
    CopyMemory HiWord, ByVal VarPtr(Param) + 2, 2
End Function
Public Function Get_X_lParam(ByVal lParam As LongPtr) As Long
Dim lpLower As Long
CopyMemory lpLower, lParam, 4 'On x86 this doesn't matter, but is needed for x64
Get_X_lParam = lpLower And &H7FFF&
If lpLower And &H8000& Then Get_X_lParam = Get_X_lParam Or &HFFFF8000
End Function
Public Function Get_Y_lParam(ByVal lParam As LongPtr) As Long
Dim lpLower As Long
CopyMemory lpLower, lParam, 4
Get_Y_lParam = (lpLower And &H7FFF0000) \ &H10000
If lpLower And &H80000000 Then Get_Y_lParam = Get_Y_lParam Or &HFFFF8000
End Function
#Else
Public Function LoWord(ByVal DWord As Long) As Integer
If DWord And &H8000& Then
...
Public Function UTF32CodePoint_To_UTF16(ByVal CodePoint As LongPtr) As String
Dim CodePointL As Long
CopyMemory CodePointL, CodePoint, 4
If CodePointL >= &HFFFF8000 And CodePoint <= &H10FFFF Then
    Dim HW As Integer, LW As Integer
    If CodePointL < &H10000 Then
        HW = 0
        LW = CUIntToInt(CodePointL And &HFFFF&)
    Else
        CodePointL = CodePointL - &H10000
        HW = (CodePointL \ &H400) + &HD800&
        LW = (CodePointL Mod &H400) + &HDC00&
    End If
...
Public Function BitmapHandleFromPicture(ByVal Picture As IPictureDisp, Optional ByVal BackColor As OLE_COLOR) As LongPtr
        ...
        If hDC <> 0 Then
            ...
            If hBmp <> 0 Then
                ...
                Else
                    ...
                    .Render hDCl Or 0&, 0&, 0&, CX Or 0&, CY Or 0&, 0&, .Height, .Width, -.Height, ByVal 0&
                    #Else
            ...
            If RenderFlag = 0 Then
                ...
                If (.Attributes And PICTURE_TRANSPARENT) = 0 Then ' Exclude GIF
                    ...
                    If Bmp.BMBitsPixel = 32 And Bmp.BMBits <> 0 Then
                        ...
                        End With
                        CopyMemory ByVal ArrPtr(B()), VarPtr(SA1D), cbPtr
                        ...
                        For i = 0 To (Abs(Bmp.BMHeight) - 1)
                            ...
                            Next j
                        Next i
                        CopyMemory ByVal ArrPtr(B()), 0&, cbPtr
                    End If
                End If
